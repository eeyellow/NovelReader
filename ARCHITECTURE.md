...
### 5. 千萬字大長篇小說效能最佳化 (章節預處理 & 虛擬分片)
1. **後端 Node.js 章節預處理與 SQLite 持久化**：
   - 書籍上傳時由伺服器預先解析正則章節結構，序列化為 `chapters_json` 存入 SQLite，避免百萬字正則阻塞前端瀏覽器主執行緒。
   - 舊書惰性補齊：若開啟尚未生成章節之舊書，後端自動解析並回填 SQLite，無縫向下相容。
2. **前端 IndexedDB `chapters_cache` 0ms 載入**：
   - 客戶端離線快取章節陣列，下次打開千萬字巨著時跳過繁重的正則掃描，0ms 秒開目錄。
3. **超長章節段落安全分片 (Virtual Chunking)**：
   - 若小說單章超過 15,000 字（如整本無章節或大卷），自動在段落換行符邊界進行平滑分割（例如「第 1 章 (1)」、「第 1 章 (2)」），確保 DOM 節點數量在任何時候皆維持輕量，徹底消除 CSS Multi-column 計算造成的掉幀與崩潰。
